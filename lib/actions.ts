...
export async function updateSettings(formData: FormData) { const user = await requireUser(); await prisma.user.update({ where: { id: user.id }, data: { name: String(formData.get("name")), currency: String(formData.get("currency")), theme: String(formData.get("theme")) as "LIGHT" | "DARK" | "SYSTEM", aiPreferences: { monthlySummaries: formData.get("monthlySummaries") === "on", spendingAlerts: formData.get("spendingAlerts") === "on", investmentTips: formData.get("investmentTips") === "on" } } }); revalidatePath("/settings"); }
